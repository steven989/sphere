class User < ActiveRecord::Base
  ...
  def get_raw_bubbles_data(connections_override=nil,json_or_not_json=false,active=true)
    ...
    if active
      result = ActiveRecord::Base.connection.exec_query("SELECT a.id, b.score_quality,b.score_time,a.first_name,a.last_name,a.email,a.photo_access_url,dense_rank() over (partition by a.user_id order by a.id)-1 as id_rank FROM connections a LEFT JOIN connection_scores b ON a.id = b.connection_id WHERE a.user_id = #{self.id} AND active='true'").inject({}) {|accumulator,result| accumulator[result["id"].to_i] = {id:result["id"],display:result["first_name"].to_s+' '+result["last_name"].to_s,size:result["score_quality"],distance:1.00/result["score_quality"].to_f,photo_url:result["photo_access_url"],email:result["email"],id_rank:result["id_rank"] }; accumulator }
